...
public class RFIDScanActivity extends Activity {
    ...
    private HashMap<String, Boolean> scanList = new HashMap<>();
    static public int last_audit_id;
    ...
    public void initProductList() {
        products = SQLRequest.SQL.getProduct();
    }
    ...
    private boolean addEPCToList(String epc, String rssi) {
        if (!TextUtils.isEmpty(epc)) {
            ...
            if (index == -1) {
                System.out.println("Invalid tag id:<" + epc + ">. Consider adding it to the database or removing it from the work area.");
            } else {
                if (!scanList.containsKey(epc)) {
                    scanList.put(epc, true);
                    products.get(index).increaseCountByOne();
                }
            }
            ...
            if (BtInventory.getText().equals(getString(R.string.btInventory))) {
                ...
                try {
                    ...
                    for(Product p : products) {
                        if (p.getCount() > 0) {
                            if (!SQLRequest.SQL.addProductAudit(last_audit_id, p.getTag_id())) {
                                UIHelper.ToastMessage(RFIDScanActivity.this, "Failed to add tag <" + p.getTag_id() + "> to database.");
                            } else {
                                successCount++;
                            }
                        }
                    ...
                    if (successCount > 0) {
                        UIHelper.ToastMessage(RFIDScanActivity.this, "Saved successfully " + successCount + " tags.");
                        tv_count.setText("0");
                        tagList.clear();
                        products.clear();
                        scanList.clear();
                        ...
                        UIHelper.ToastMessage(RFIDScanActivity.this, "Failed to save. Check your connection");
                    }
                }
                catch (Exception ex) {
                    UIHelper.showExceptionError(RFIDScanActivity.this, ex);
...
//                }
    ...
    private void readTag() {
        if (BtInventory.getText().equals(getString(R.string.btInventory)))
        {
            ...
            int result = SQLRequest.SQL.addAuditReport();
            if (result != -1) {
                last_audit_id = result;
            } else {
                UIHelper.ToastMessage(RFIDScanActivity.this, "Failed to add new audit report.");
                return;
            }
        ...
        public void run() {
            ...
            while (loopFlag) {
                ...
                if (res != null) {
                    ...
                    msg.obj = res.getEPC() + "@" + res.getRssi();
